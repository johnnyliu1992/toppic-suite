<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
	...
	import="edu.iupui.iac.msalignplus.dao.*" pageEncoding="ISO-8859-1"%>
...
<script type="text/javascript">
	...
	function refreshRow(ids) {
		...
		htmlobj = $.ajax({
			url : "runningPercent.do?name=" + ids,
			async : false
		});
		...
		if ((arr[1] == "finished" && arr[2] == "1") || arr[1] == "waiting"
				|| arr[1] == "error") {
...
<body>
	<table class="table table-bordered" style="width: 820px;">
		<thead>
			<tr>
				...
				<td align="center" title='hours:minutes:seconds'>Time</td>
				<td align="center">Progress</td>
				<td><button type='button' class='btn btn-success btn-xs'
						onclick="window.location.reload()">Refresh All</button></td>
			</tr>
		</thead>
		<tbody>
			<%
				...
				String servPath = request.getSession().getServletContext()
						.getRealPath("/");
				...
				if (fileName != null) {
					for (int i = 0; i < fileName.length; i++) {
						File argumentXML = new File(servPath + "result/"
								+ fileName[i] + "/" + "arguments.xml");
						if (!argumentXML.exists())
							continue;

						String prog_name = MsDeconvArgument.getProgramName(servPath
								+ "result/" + fileName[i] + "/", "arguments.xml");
						File taskEndFile = new File(servPath + "result/"
								+ fileName[i] + "/log.txt");
						double per = 0.00;
						String status = "waiting";
						if (prog_name.equals("topid")) {
							TopIDargument arg = TopIDargument.getArgument(servPath
									+ "result/" + fileName[i] + "/",
									"arguments.xml");
							if (taskEndFile.exists()) {
								...
								//read file 
								try {
									reader = new FileReader(servPath + "result/"
											+ fileName[i] + "/log.txt");
									...
									if (status.equalsIgnoreCase("finished")) {
										if (null != arg) {
											String foldername = arg
													.getSpectrum_File().replace(
															".msalign", "");
											out.println("<a href='"
													+ path
													+ "/result/"
													...
													+ (arg == null ? "#"
															: foldername)
													+ "_html/proteins.html' target='_blank'>");
										}
									...
									if (status.equalsIgnoreCase("error")
											|| status
													.equalsIgnoreCase("force stop")) {
										out.println("<font color='red'>"
												+ (arg == null ? "File Error" : arg
														.getTitle())
												+ "<span id='P" + fileName[i]
												+ "'>(" + status
												+ ")</span></font>");
									} else {
										out.println(""
									...
									out.println("</td><td align='center' width='15%'>Topid</td><td align='center' width='20%'>"
											+ formattor.format(new Date(
													arg == null ? 0 : arg
															.getStart())) + "</td>");
									if (status.equalsIgnoreCase("waiting")) {
										out.println("<td id='TDC"
												+ fileName[i]
												+ "' widht='12%' align='right' title='"
										...
										out.println("<td id='TDC"
												+ fileName[i]
												+ "' widht='12%' align='right' title='"
												+ (arg == null ? "0 hour 0 minute 0 second"
														: arg.getCostComment())
												+ "'>"
												+ (arg == null ? "0:00:00" : arg
														.getCostString()) + "</td>");
									}

									out.println("<td id='TD" + fileName[i]
									...
									if (status.equalsIgnoreCase("running")
											|| status.equalsIgnoreCase("waiting")) {
										out.println("<button type='button' id='BtnR"
												+ fileName[i]
										...
										out.println("<button type='button' id='BtnG"
												+ fileName[i]
												+ "' class='btn btn-primary btn-xs' style='display:none' onclick=downloadRow('"
												+ fileName[i]
												+ "')>Download</button>");
										if (status.equalsIgnoreCase("waiting")) {
											out.println("<button type='button' id='BtnD"
													+ fileName[i]
													+ "' class='btn btn-warning btn-xs' onclick=toDeleteResult('"
													+ fileName[i]
													+ "',this)>Delete</button>");
										} else {
											out.println("<button type='button' id='BtnD"
										...
										out.println("<button type='button' id='BtnR"
												+ fileName[i]
												+ "' class='btn btn-success btn-xs' style='display:none' onclick=refreshRow('"
												+ fileName[i]
												+ "')>&nbsp;&nbsp;Refresh&nbsp;&nbsp;</button>");
										...
										out.println("<button type='button' id='BtnG"
												+ fileName[i]
												+ "' class='btn btn-primary btn-xs'  onclick=downloadRow('"
												+ fileName[i]
												+ "')>Download</button>");
										out.println("<button type='button' id='BtnD"
												+ fileName[i]
								...
								out.println("<tr><td width='10%' valign='top'>"
										...
										+ "</td><td width='20%' valign='top'>");
								out.println("<a href='" + path
										+ "/fileList.do?path=/" + fileName[i]
										+ "' target='_blank'>");
								out.println(""
										+ (arg == null ? "File Error" : arg
												.getTitle()) + "<span id='P"
										+ fileName[i] + "'>(" + status + ")</span>");
								out.println("</a><br/><div style='display:none'>aaa</div>");
								out.println("</td><td align='center'  width='15%'>Topid</td><td align='center' width='20%'>"
										+ formattor.format(new Date(arg == null ? 0
												: arg.getStart())) + "</td>");
								out.println("<td id='TDC" + fileName[i]
										+ "' widht='12%' align='right' title='"
										+ "0 hour 0 minute 0 second" + "'>"
										+ "0:00:00" + "</td>");
							...
							if (taskEndFile.exists()) {
								...
								try {
									...
									while (((str = br.readLine()) != null)
										...
										if (str.contains("parameters end")) {
											...
											status = "running";
												...
												.startsWith("Processing spectrum")) {
											String[] temp = str.split(" ");
											str = temp[3];
											per = Double.parseDouble(str.substring(
													0, str.length() - 1));
											per = 10.00 + per * 0.9;
											status = "running";
									...
									out.println("</a><br/><div style='display:none'>aaa</div>");
									out.println("</td><td align='center' width='15%'>MS-Deconv+</td><td align='center' width='20%'>"
											+ formattor.format(new Date(
													arg == null ? 0 : arg
															.getStart())) + "</td>");
									if (status.equalsIgnoreCase("waiting")) {
	...
	<form id="deleteFrm" action="deleteTask.do" method="post">
		<input type="hidden" name="delName" id="delName_id" /> <input
			type="hidden" name="isRunning" id="runningStats" />
	</form>
